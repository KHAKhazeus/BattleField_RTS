...
+ 如果有一方设备性能低下，可能会导致双方无法正常同时进入游戏，进而会导致延迟增加。建议使用高性能设备或重新尝试进入游戏。
+ 在游戏菜单中如果已经点击Start Server或Start Client请勿再点Reset Client/Server或Return。
+ 如果你试图建造建筑和生产单位时没有反应，请检查电力和金钱是否足够建造和生产。
+ 如果你的电厂被摧毁，当电量值达到或超过一个电厂的电力值时将减去相应电力值，当电量不足一个电厂的电力值时电力值将直接清零。但不会影响已建造建筑的生产。
+ 如果点击基地没有出现建筑选择，可能是因为你连续点击了，请框选一块空地取消选择后重试一次。
+ 因为建筑被攻击后无法修复，所以血条将不会消失以提醒你小心敌人攻击，别担心，这是正常现象。
